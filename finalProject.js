...
import('./src/moviesPlay.js')
    .then(res => {
        ...
        //SelectedMovieDetails(queryParamsMap.get('id'),queryParamsMap.get('posterPath'));
        showGenres();
    });
...
function setCondition() {
    ...
    if (andRadio.checked) {
        ...
        // Default to "OR" if neither radio button is checked
        console.log('inside else condition line 435');
        selectedCondition = "OR";
        orRadio.checked = true; // Set "OR" as the default
    }
...
function handleDecadeChange() {
    ...
    movieIds = getMovieIdsByDecade(selectedDecade);
     // Filter movie IDs by language (e.g., English, Hindi, All)
     movieIds = movieIds.filter(id => moviesIdbyLanguage.includes(id))
    console.log('Movie IDs by decade :', movieIds);
    getMovieInformation();
    //setMovieFilter(document.querySelector('input[name="frequency"]:checked').value);
}
